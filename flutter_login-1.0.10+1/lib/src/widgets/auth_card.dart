...
class _LoginCardState extends State<_LoginCard> with TickerProviderStateMixin {
  ...
  var _authSignupData = {'email': '', 'password': '', 'cofirmPassword': ''};
  ...
  Future<bool> _submit() async {
    ...
    if (auth.isLogin) {
      ...
      error = await auth.onSignup(SignupData(
        name: _authSignupData['email'],
        password: _authSignupData['password'],
        confirmPassword: _authSignupData['confirmPassword']
      ));
    }
  ...
  Widget _buildNameField(double width, LoginMessages messages) {
    return AnimatedTextFormField(
      ...
      onSaved: (value) => _authSignupData['email'] = _authData['email'] = value,
    );
  }
  ...
  Widget _buildPasswordField(double width, LoginMessages messages) {
    ...
    return AnimatedPasswordTextFormField(
      ...
      onSaved: (value) => _authSignupData['password'] = _authData['password'] = value,
    );
  }
  ...
  Widget _buildConfirmPasswordField(double width, LoginMessages messages) {
    ...
    return AnimatedPasswordTextFormField(
      ...
      validator: auth.isSignup
          ...
          : (value) => null,
        onSaved: (value) => _authSignupData['confirmPassword'] = value,   
    );
  }
...
class _RecoverCardState extends State<_RecoverCard>
  ...
  Future<bool> _submit() async {
    ...
    _submitController.forward();
    setState(() => _isSubmitting = true);
    final error = await auth.onRecoverPassword(_email);
